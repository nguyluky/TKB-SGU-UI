...
import { ApiResponseBase } from '~/api/Api';
import Err from '../Err';
...
function TkbBody() {
    ...
    React.useEffect(() => {
        if (tkb instanceof ApiResponseBase) {
            if (tkb.code === 403) {
                setErr('403');
            }
        } else {
            ...
            tkbDispath({ path: 'currTkb', value: tkb });
        }

        return () => {
            tkbDispath({ path: 'mahp_idtohoc', value: null });
                                                    ...
                                                    return (
                                                        <div className="hp" key={i}>
                                                            <label className="container">
                                                                <input
                                                                    type="checkbox"
                                                                    checked={tkbState.currTkb?.ma_hoc_phans.includes(
                                                                        e[0],
                                                                    )}
                                                                    onChange={(event) => {
                                                                        if (event.target.checked) {
                                                                            ...
                                                                            const index =
                                                                                tkbState.currTkb?.ma_hoc_phans.indexOf(
                                                                                    e[0],
                                                                                );
                                                                            if (index > -1) {
                                                                                // only splice array when item is found
                                                                                tkbState.currTkb?.ma_hoc_phans.splice(
                                                                                    index,
                                                                                    1,
